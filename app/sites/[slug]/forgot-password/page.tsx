...
export default async function ForgotPasswordPage({ params, searchParams }: { params: { slug: string }, searchParams: { checkout?: string } }) {
    ...
    return (
        <div style={{ background: template.colors.bg }} className="pt-24 min-h-screen flex items-center justify-center p-6">
            <ForgotPasswordForm business={business} template={template} slug={params.slug} nextUrl={nextUrl} />
        </div>
    );
}
