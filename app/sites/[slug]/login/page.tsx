...
export default async function LoginPage({ params, searchParams }: { params: { slug: string }, searchParams: { checkout?: string } }) {
    ...
    return (
        <div style={{ background: template.colors.bg }} className="pt-24 min-h-screen flex items-center justify-center p-6">
            <LoginForm business={business} template={template} slug={params.slug} nextUrl={nextUrl} />
        </div>
    );
}
