...
export default async function NewServicePage() {
    ...
    const { data: business } = await db.from("businesses").select("id, business_type, services_label").eq("slug", slug).single();
    ...
    return (
        <div className="max-w-2xl space-y-6 pb-12">
            <div className="flex items-center space-x-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold">New {config.singular}</h2>
                    <p className="text-muted-foreground text-sm">Add a new {config.singular.toLowerCase()} to your {config.singular === 'Service' ? 'menu' : 'store'}.</p>
                </div>
            </div>
                ...
                <form action={upsertService} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <Input id="title" name="title" placeholder={`e.g., ${config.singular === 'Product' ? 'Denim Jacket' : 'Bridal Makeup'}`} required className="h-11 rounded-xl" />
                        </div>
                        <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <Input id="tags" name="tags" placeholder="e.g. Micro, Tech, 100k+ Followers" className="h-11 rounded-xl" />
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="features" name="features" placeholder="e.g. 1x Reel, 2x Stories" className="h-11 rounded-xl" />
                            <p className="text-[10px] text-muted-foreground italic">Separate with commas</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Textarea id="description" name="description" placeholder={`Describe the ${config.singular.toLowerCase()}...`} rows={4} className="rounded-xl resize-none" />
                    </div>
